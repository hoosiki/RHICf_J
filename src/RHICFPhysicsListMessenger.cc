...
RHICFPhysicsListMessenger::RHICFPhysicsListMessenger(RHICFPhysicsList* pPhys) : fPhysicsList(pPhys)
{

    fDirectory = new G4UIdirectory("/RHICfPhysicsList/");
    fDirectory->SetGuidance("RHICFPhysicsList control");
 
    fSetAbsorptionCMD = new G4UIcmdWithABool("/RHICfPhysicsList/setAbsorption", this);
    fSetAbsorptionCMD->SetGuidance("Turn on or off absorption process");
    fSetAbsorptionCMD->AvailableForStates(G4State_Idle);

    fVerboseCmd = new G4UIcmdWithAnInteger("/RHICfPhysicsList/verbose",this);
    ...
    fCerenkovCmd = new G4UIcmdWithAnInteger("/RHICfPhysicsList/cerenkovMaxPhotons",this);
    ...
    fGammaCutCMD = new G4UIcmdWithADoubleAndUnit("/RHICfPhysicsList/gammaCut",this);
    ...
    fElectCutCMD = new G4UIcmdWithADoubleAndUnit("/RHICfPhysicsList/electronCut",this);
    ...
    fPosCutCMD = new G4UIcmdWithADoubleAndUnit("/RHICfPhysicsList/positronCut",this);
    ...
    fAllCutCMD = new G4UIcmdWithADoubleAndUnit("/RHICfPhysicsList/allCuts",this);
    ...
    fStepMaxCMD = new G4UIcmdWithADoubleAndUnit("/RHICfPhysicsList/stepMax",this);
    ...
    fClearPhysicsCMD = new G4UIcmdWithoutParameter("/RHICfPhysicsList/clearPhysics",this);
    fClearPhysicsCMD->SetGuidance("Clear the physics list");
    fClearPhysicsCMD->AvailableForStates(G4State_PreInit,G4State_Idle);

    fRemovePhysicsCMD = new G4UIcmdWithAString("/RHICfPhysicsList/removePhysics",this);
    ...
    fListCMD = new G4UIcmdWithoutParameter("/RHICfPhysicsList/list",this);
                                          ...
                                          G4String newValue)
{
    if( command == fSetAbsorptionCMD ) {
       ...
       G4VDecayChannel* mode = new G4PhaseSpaceDecayChannel("pi+",1.0,2,"e+","nu_e");
       ...
       G4ParticleTable* particleTable = G4ParticleTable::GetParticleTable();
       G4ParticleDefinition* particleDef = particleTable->FindParticle("pi+");
       G4VDecayChannel* mode = new G4PhaseSpaceDecayChannel("pi+",1.000,2,"mu+","nu_mu");
       G4DecayTable* table = new G4DecayTable();
       table->Insert(mode);
       particleDef->SetDecayTable(table);
    }
